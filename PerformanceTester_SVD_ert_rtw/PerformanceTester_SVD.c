 ...
 * C/C++ source code generated on : Sun Nov 15 18:27:21 2020
 *
...
static void isfinite(const real_T x[3000], boolean_T b[3000]);
static real_T xnrm2(int32_T n, const real_T x[3000], int32_T ix0);
static real_T xdotc(int32_T n, const real_T x[3000], int32_T ix0, const real_T
                    y[3000], int32_T iy0);
static void xaxpy(int32_T n, real_T a, int32_T ix0, const real_T y[3000],
                  int32_T iy0, real_T b_y[3000]);
static real_T xnrm2_n(int32_T n, const real_T x[50], int32_T ix0);
static void xaxpy_n5iz(int32_T n, real_T a, const real_T x[3000], int32_T ix0,
  real_T y[60], int32_T iy0);
static void xaxpy_n5(int32_T n, real_T a, const real_T x[60], int32_T ix0, const
                     real_T y[3000], int32_T iy0, real_T b_y[3000]);
...
static void xswap(const real_T x[2500], int32_T ix0, int32_T iy0, real_T b_x
                  [2500]);
static void xswap_n(const real_T x[3000], int32_T ix0, int32_T iy0, real_T b_x
                    [3000]);
...
static void xrot_n(const real_T x[3000], int32_T ix0, int32_T iy0, real_T c,
                   real_T s, real_T b_x[3000]);
static void svd_n(const real_T A[3000], real_T U[3000], real_T s[50], real_T V
                  [2500]);
static void svd(const real_T A[3000], real_T U[3000], real_T S[2500], real_T V
                [2500]);
static void SystemCore_step(const real_T varargin_1[3000], real_T varargout_1
  [3000], real_T varargout_2[50], real_T varargout_3[2500]);
                  ...
                  int32_T iy0, real_T b_y[3000])
{
  ...
  int32_T iy;
  int32_T k;
  memcpy(&b_y[0], &y[0], 3000U * sizeof(real_T));
  if (!(a == 0.0)) {
    ix = ix0;
    iy = iy0 - 1;
    for (k = 0; k < n; k++) {
                    ...
                    [3000])
{
  ...
  int32_T iy;
  int32_T k;
  memcpy(&b_x[0], &x[0], 3000U * sizeof(real_T));
  ix = ix0 - 1;
  iy = iy0 - 1;
  for (k = 0; k < 60; k++) {
    temp = b_x[ix];
                  ...
                  [2500])
{
  ...
  boolean_T exitg1;
  memcpy(&rtDW.A[0], &A[0], 3000U * sizeof(real_T));
  memset(&s_0[0], 0, 50U * sizeof(real_T));
  memset(&e[0], 0, 50U * sizeof(real_T));
  memset(&work[0], 0, 60U * sizeof(real_T));
  memset(&U[0], 0, 3000U * sizeof(real_T));
  ...
  for (m = 0; m < 50; m++) {
    ...
    nrm = xnrm2(60 - m, rtDW.A, colqp1 + 1);
    if (nrm > 0.0) {
    ...
    for (qp1jj = qp1; qp1jj < 51; qp1jj++) {
      qjj = ((qp1jj - 1) * 60 + m) + 1;
      if (apply_transform) {
        memcpy(&rtDW.A_m[0], &rtDW.A[0], 3000U * sizeof(real_T));
        xaxpy(60 - m, -(xdotc(60 - m, rtDW.A, colqp1 + 1, rtDW.A, qjj) /
    ...
    if (m + 1 <= 48) {
      ...
      if (nrm == 0.0) {
        ...
        for (qq = qp1; qq < 51; qq++) {
          xaxpy_n5iz(59 - m, e[qq - 1], rtDW.A, (m + 60 * (qq - 1)) + 2, work, m
                     + 2);
        }

        for (qq = qp1; qq < 51; qq++) {
          memcpy(&rtDW.A_m[0], &rtDW.A[0], 3000U * sizeof(real_T));
          xaxpy_n5(59 - m, -e[qq - 1] / e[m + 1], work, m + 2, rtDW.A_m, ((qq -
                     1) * 60 + m) + 2, rtDW.A);
        }
      }
  ...
  for (colqp1 = 49; colqp1 >= 0; colqp1--) {
    ...
    if (s_0[colqp1] != 0.0) {
      for (qp1jj = colqp1 + 2; qp1jj < 51; qp1jj++) {
        qjj = ((qp1jj - 1) * 60 + colqp1) + 1;
        memcpy(&rtDW.A[0], &U[0], 3000U * sizeof(real_T));
        xaxpy(60 - colqp1, -(xdotc(60 - colqp1, U, qq + 1, U, qjj) / U[qq]), qq
              + 1, rtDW.A, qjj, U);
      }

      for (qp1 = colqp1 + 1; qp1 < 61; qp1++) {
        qp1jj = (60 * colqp1 + qp1) - 1;
        U[qp1jj] = -U[qp1jj];
      }

      U[qq]++;
      if (0 <= colqp1 - 1) {
        memset(&U[colqp1 * 60], 0, ((colqp1 - 1) + 1) * sizeof(real_T));
      }
    } else {
      memset(&U[colqp1 * 60], 0, 60U * sizeof(real_T));
      U[qq] = 1.0;
    }
  }
    ...
    switch ((int32_T)ztest0) {
     ...
     case 2:
      ...
      for (qp1 = colqp1 + 1; qp1 <= m + 1; qp1++) {
        ...
        memcpy(&rtDW.A[0], &U[0], 3000U * sizeof(real_T));
        xrot_n(rtDW.A, (qp1 - 1) * 60 + 1, (colqp1 - 1) * 60 + 1, sqds, smm1, U);
      }
      break;
     ...
     case 3:
      ...
      for (qq = colqp1 + 1; qq <= m; qq++) {
        ...
        e[qq] *= smm1;
        memcpy(&rtDW.A[0], &U[0], 3000U * sizeof(real_T));
        xrot_n(rtDW.A, (qq - 1) * 60 + 1, 60 * qq + 1, smm1, emm1, U);
      }
     ...
     default:
      ...
      while ((colqp1 + 1 < 50) && (s_0[colqp1] < s_0[qp1])) {
        ...
        memcpy(&rtDW.A[0], &U[0], 3000U * sizeof(real_T));
        xswap_n(rtDW.A, 60 * colqp1 + 1, (colqp1 + 1) * 60 + 1, U);
        colqp1 = qp1;
        qp1++;
      }
  ...
  [3000], real_T varargout_2[50], real_T varargout_3[2500])
{
  ...
  if (y) {
    ...
    for (b_k = 0; b_k < 50; b_k++) {
      varargout_2[b_k] = rtDW.s[50 * b_k + b_k];
    }
  } else {
    memset(&varargout_1[0], 0, 3000U * sizeof(real_T));
